...
def handle_live_pipeline(inp: dict) -> Generator[dict, None, None]:
    ...
    if target_lang != src_lang:
        if pre_translated:
            translated = pre_translated  # GPT-4o fait côté Hostinger
        else:
            try:
                if translation_provider == "opus-mt":
                    ...
                    log.warning("Unknown trad provider %s, no translation", translation_provider)
            except Exception as e:
    ...
    try:
        if mode == "gpu-clone":
            if not voice_ref_b64:
                yield {"type": "error",
                       "message": "voice_ref required for gpu-clone mode"}
                return
            ...
            # Phase 1 : F5-TTS native (synthèse complète, pas de streaming car
            # RVC veut traiter l'audio complet)
            native_audio_array = get_f5tts().synthesize_native(
                text_to_speak, target_lang
            )
            
            # Phase 2 : RVC streaming
...
# ============================================================================
# Operation: rvc_convert (mode fichier)
# ============================================================================
