"""Wrapper OPUS-MT GPU (Helsinki-NLP) pour traduction rapide multi-paires.
...
Pour étendre, ajouter dans MODEL_NAMES et télécharger le modèle correspondant
sur HuggingFace.

Optimisation : utilise CTranslate2 pour des perfs ~3x meilleures que
transformers MarianMT pur.
"""
...
# Paires de langues supportées et nom du modèle HF correspondant
...
class OpusMT:
    ...
    def __init__(self, max_cached_models: int = 4):
        ...
        # Tente de charger CTranslate2, fallback transformers
        try:
            import ctranslate2
            self.use_ct2 = True
    ...
    def _load_ct2(self, model_name: str) -> dict:
        """CTranslate2 nécessite un modèle pré-converti.
        
        Pour V3 launch : on convertit à la première utilisation et on cache.
        """
        import ctranslate2
    ...
    def _translate_ct2(self, text: str, bundle: dict) -> str:
        ...
        results = translator.translate_batch([tokens], beam_size=1, max_decoding_length=512)
        translated_tokens = results[0].hypotheses[0]
        translated_text = tok.convert_tokens_to_string(translated_tokens)
